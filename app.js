...
connection.query(
  "SELECT * FROM juegos WHERE promocion > 0",
  async (error, results) => {
    for (let i = 0; i < results.length; i++) {
      var juego = new Juego(
        ...
        results[i].promocion,
        results[i].destacado,
        results[i].proximamente,
        results[i].recomendado,
        results[i].progreso
      );
...
connection.query(
  "SELECT * FROM juegos WHERE destacado = 1",
  async (error, results) => {
    for (let i = 0; i < results.length; i++) {
      var juego = new Juego(
        results[i].id,
...
//Leer juegos destacados
connection.query(
  "SELECT * FROM juegos WHERE recomendado = 1",
  async (error, results) => {
    for (let i = 0; i < results.length; i++) {
      var juego = new Juego(
        results[i].id,
...
connection.query(
  "SELECT * FROM juegos WHERE proximamente = 1",
  async (error, results) => {
    for (let i = 0; i < results.length; i++) {
      var juego = new Juego(
        results[i].id,
...
connection.query(
  "SELECT * FROM juegos WHERE precio < 20000",
  async (error, results) => {
    for (let i = 0; i < results.length; i++) {
      var juego = new Juego(
        results[i].id,
...
function cargarCarrito(req) {
  ...
  totalCarritoActual = 0;
  connection.query(
    "SELECT * FROM usersxjuegos WHERE id_usuario = ?",
    [req.session.idActual],
    async (error, results) => {
      for (let i = 0; i < results.length; i++) {
        connection.query(
...
app.post("/eliminar-Juego", async (req, res) => {
  ...
  connection.query(
    "DELETE FROM usersxjuegos WHERE id_usuario = ? AND id_juego = ?",
    [req.session.idActual, idJuego],
    async (error, results) => {
  ...
  connection.query(
    ...
    async (error, results) => {
      ...
      for (let i = 0; i < results.length; i++) {
        connection.query(
          "DELETE FROM usersxjuegos WHERE id_usuario = ? AND id_juego = ?",
          [req.session.idActual, results[i].id_juego],
          async (error, results) => {
            if (error) return console.error(error.message);
            console.log("Deleted Row(s):", results.affectedRows);
            cargarCarrito(req);
...
app.post("/buscar-Juego", async (req, res) => {
  ...
  connection.query(
    "SELECT * FROM juegos WHERE nombre = ?",
    [busqueda],
    async (error, results) => {
      if (results.length == 0) {
        if (req.session.loggedin) {
...
app.post("/agregar-Juego", async (req, res) => {
  const { idJuego } = req.body;
  connection.query(
    "INSERT INTO usersxjuegos(id_usuario, id_juego) VALUES (?,?)",
    [req.session.idActual, idJuego],
    async (error, results) => {
      if (error) return console.error(error.message);
